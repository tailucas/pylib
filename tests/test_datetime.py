...
from tailucas_pylib.datetime import make_timestamp, make_iso_timestamp, make_unix_timestamp
...
def test_make_timestamp(setup_datetime_datetime_notz, setup_datetime_datetime, setup_datetime_string):
    assert make_timestamp() is not None
    assert make_timestamp(timestamp=setup_datetime_datetime_notz) == setup_datetime_datetime
    edt_timestamp: datetime = make_timestamp(timestamp=setup_datetime_datetime_notz, as_tz=timezone('US/Eastern'))
...
def test_make_iso_timestamp(setup_datetime_datetime_notz, setup_datetime_datetime, setup_datetime_string):
    assert make_iso_timestamp() is not None
    assert make_iso_timestamp(timestamp=setup_datetime_datetime) == "1985-10-26T01:21:00Z"
    assert make_iso_timestamp(timestamp=setup_datetime_string) == "1985-10-26T01:21:00Z"
    assert make_iso_timestamp(timestamp=setup_datetime_datetime_notz, as_tz=timezone('US/Eastern')) == "1985-10-25T21:21:00-04:00"
