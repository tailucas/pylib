...
try:
    ...
    log.warning(
        f'Logging will be sent directly to remote address {syslog_address}')
...
config.log = log  # type: ignore
config.log_handler = log_handler  # type: ignore
...
if current_work_dir != WORK_DIR:
    log.warning(
        f"Changing working directory from {current_work_dir} to {WORK_DIR}")
    os.chdir(WORK_DIR)
...
if os.path.exists(app_config_path) and os.path.getsize(app_config_path) > 0:
    ...
    config.DEVICE_NAME = device_name  # type: ignore
    ...
    config.DEVICE_NAME_BASE = device_name_base  # type: ignore
else:
    log.warning(
        f'Setting config.DEVICE_NAME and config.DEVICE_NAME_BASE to "{APP_NAME}" due to missing configuration.'
    )
    config.DEVICE_NAME = APP_NAME
    config.DEVICE_NAME_BASE = APP_NAME
config.app_config = app_config  # type: ignore
